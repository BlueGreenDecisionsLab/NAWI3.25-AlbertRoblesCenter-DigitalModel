...
#====== DATA IMPORT =================
#Import Excel sheet
data = read_excel(path = "sweep_results2/results.xlsx")

#Add any additional parameters (e.g. summer demand charge)
data %>% mutate(
  on_peak_demand_charge_summer = max_on_peak_demand_kW * 19.77,
  off_peak_demand_charge_summer = max_off_peak_demand_kW * 23.94,
  net_demand_charge_dol_summer = on_peak_demand_charge_summer + off_peak_demand_charge_summer
)
#====================================

#===========================================
#====== PLOT 1: Energy Intensity Plot=======
...
demandCharge = read_excel("_RESULTS.xlsx", 
                          sheet = "RelativeCostChange-Demand", range = "A1:F19")
...
ggplot(df_long, aes(x = EnergyIntensity, y = Solar, color = Controller)) +
  ...
  labs(
    title    = 'Solar and Operational Scenario Energy Intensity',
    x        = 'Energy Intensity, kWh/MG Permeate',
    y        = 'Solar Generation Scenario',
  ) +
...
ggsave("Visualizations/EnergyIntensity.png", width = 10, height = 6)
#==== Demand CHarge Plot ====
...
df_long <- demandCharge %>%
  ...
  mutate(
    ...
    Season = Season,
    # Delta from Steady State within each solar + season group
...
# --- Build ref from Steady State rows ---
ref_existing <- df_existing %>%
  filter(Controller == 'Steady State') %>%
  select(Season, Solar, Rel_Demand_Charge)

# --- Plot: Both Seasons ---
ggplot(df_existing, aes(x = Rel_Demand_Charge, y = Season, color = Controller)) +
  
  # Connecting line between dots
  geom_line(aes(group = Season), color = 'grey70', linewidth = 0.8) +
  ...
  labs(
    ...
    subtitle = 'Existing Solar (0.3 MW) — All Seasons',
...
ggsave("Visualizations/RelDem.png", width = 8, height = 5)
...
df_long_mg <- costperMG %>%
  ...
  mutate(
    ...
    TOU = factor(recode(as.character(TOU),
                        ...
                        'No TOU'       = 'Static TOU'),
                 levels = c('Static TOU','Existing TOU Rates', 'Greater TOU Variance (+50%)')),
...
ggplot(df_plot, aes(x = as.numeric(Controller), fill = Direction)) +
  ...
  geom_text(
    aes(x     = as.numeric(Controller),
        y     = ifelse(Delta >= 0, max(bar_top + 0.3,.1), min(bar_bottom - 0.3,-.1)),
  ...
  theme(
    ...
    plot.title         = element_text(face = 'bold'),
    axis.text.x        = element_text(size = 16, lineheight = 1.3),
