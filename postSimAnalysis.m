...
total_energy_consumption_kWh = abs(simOut.power.Plant_TotalPowerDemand_kWh.Data(end));
total_energy_cost_dol = simOut.power.Plant_TotalEnergyCosts_dol.Data(end);

%3) Brine costs
total_brine_cost_dol = simOut.flows.Plant_TotalBrineProduction_MG.Data(end);
...
flows = simOut.flows;
permeate_target = 62.23; %Million Gallons
...
xlabel('Simulation Time (min)')
ylabel('Power (kW)')
title('Power Demand, Solar, Battery, and Net')
legend([p1 p2 p3 a1], {'Net Power Demand','Solar Generation','Battery Power','Net (Demand - Solar + Battery)'}, 'Location','best')
grid on
